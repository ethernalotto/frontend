...
const MaybeNumberEditor = ({numbers, onAddNumber}) => {
  const [newNumber, setNewNumber] = useState(null);
  const maybeAddNumber = text => {
    if (!/^[0-9]+$/.test(text)) {
      return;
    }
  ...
  if (newNumber !== null) {
    return (
      <form className="list-activated__item" onSubmit={e => {
        e.preventDefault();
    ...
    return (
      <div className="list-activated__item">
        <button className="btn btn-plus btn-small-arrows" onClick={() => {
          setNewNumber('');
        }}>
...
const NumberList = ({numbers, splice, onAddNumber}) => (
  ...
  useEffect(() => {
    (async () => {
      ...
      if (lottery && numbers.length >= 6) {
        ...
        setPrice(Math.round(priceValue * 10000) / 10000);
      }
    })();
  }, [lottery, numbers]);
  return (
    <div className="lucky-statistic d-flex justify-content-around align-items-center">
      ...
      <div className="lucky-statistic__cost">
        <div className="lucky-statistic__title">Price (ETH)</div>
